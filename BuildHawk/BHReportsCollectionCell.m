...
- (void)configureForReport:(Report *)report withDateFormatter:(NSDateFormatter *)dateFormatter andNumberFormatter:(NSNumberFormatter *)number withTimeStampFormatter:(NSDateFormatter *)timeStamp withCommentFormatter:(NSDateFormatter *)comment withWidth:(CGFloat)w andHeight:(CGFloat)h {
    ...
    NSLog(@"configuring report with %d safety topics",_report.safetyTopics.count);
...
- (NSInteger)tableView:(BHReportTableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (section == 0 /*&& [_report.identifier isEqualToNumber:[NSNumber numberWithInt:0]]*/){
        return  1;
        ...
        return _report.reportSubs.count;
    } else if (section == 5 && [_report.type isEqualToString:kSafety]){
        return 1;
    } else if (section == 6 && [_report.type isEqualToString:kSafety]){
        ...
        if (activities){
            if ([_report.type isEqualToString:kDaily]){
                return _report.dailyActivities.count;
            } else {
                return _report.activities.count;
            }
        } else {
            return _report.comments.count + 1;
        }
    } else {
        return 0;
    }
...
- (UITableViewCell *)tableView:(BHReportTableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        /*if ([_report.saved isEqualToNumber:@NO]){
        ...
        if (_report.reportUsers.count > indexPath.row){
            ReportUser *reportUser = [_report.reportUsers objectAtIndex:indexPath.row];
        ...
        if (_report.reportSubs.count > indexPath.row){
            ReportSub *reportSub = [_report.reportSubs objectAtIndex:indexPath.row];
        ...
        if (_report.safetyTopics.count > indexPath.row){
            SafetyTopic *topic = [_report.safetyTopics objectAtIndex:indexPath.row];
        ...
        if (activities){
            ...
            if ([_report.type isEqualToString:kDaily]){
                Activity *activity = _report.dailyActivities[indexPath.row];
            ...
            } else if (_report.activities.count > indexPath.row) {
                Activity *activity = _report.activities[indexPath.row];
                [cell.timestampLabel setText:[timeStampFormatter stringFromDate:activity.createdDate]];
            ...
            if (indexPath.row == 0){
                ...
                Comment *comment = [_report.comments objectAtIndex:indexPath.row - 1];
...
- (CGFloat)tableView:(BHReportTableView *)tableView heightForHeaderInSection:(NSInteger)section {
    /*if (section == 0 && ![_report.identifier isEqualToNumber:[NSNumber numberWithInt:0]]){
        return 0;
    } else */if (section == 3 && !_report.reportUsers.count){
        return 0;
    } else if (section == 4 && !_report.reportSubs.count){
        return 0;
    } else if (section == 1 && ![_report.type isEqualToString:kDaily]){
        return 0;
    } else if (section == 5 && ![_report.type isEqualToString:kSafety]){
        ...
        if ([_report.type isEqualToString:kDaily] && _report.dailyActivities.count == 0){
            return 0;
        } else if (_report.activities.count == 0){
            return 0;
        } else {
            return 40;
        }
...
- (UIView*)tableView:(BHReportTableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (![_report.type isEqualToString:kDaily] && section == 1){
    ...
    }/* else if (section == 0 && ![_report.identifier isEqualToNumber:[NSNumber numberWithInt:0]]){
        return [[UIView alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
    }*/ else if (section == 9) {
        ...
        if ([_report.type isEqualToString:kDaily]){
            activitiesTitle = _report.dailyActivities.count == 1 ? @"1 ACTIVITY" : [NSString stringWithFormat:@"%lu ACTIVITIES",(unsigned long)_report.dailyActivities.count];
        } else {
            activitiesTitle = _report.activities.count == 1 ? @"1 ACTIVITY" : [NSString stringWithFormat:@"%lu ACTIVITIES",(unsigned long)_report.activities.count];
        }
        ...
        NSString *commentsTitle = _report.comments.count == 1 ? @"1 COMMENT" : [NSString stringWithFormat:@"%lu COMMENTS",(unsigned long)_report.comments.count];
        ...
        switch (section) {
            ...
            case 9:
                if ([_report.type isEqualToString:kDaily]){
                    ...
                    [headerLabel setText:[NSString stringWithFormat:@"%@ ACTIVITY",_report.project.name.uppercaseString]];
                }
                activityButton = [UIButton buttonWithType:UIButtonTypeCustom];
                [activityButton.titleLabel setTextAlignment:NSTextAlignmentLeft];
                [activityButton.titleLabel setFont:[UIFont fontWithName:kMyriadPro size:14]];
...
- (void)tableView:(BHReportTableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 6 && [_report.type isEqualToString:kSafety]){
        if (_report.safetyTopics.count > indexPath.row){
            SafetyTopic *topic = [_report.safetyTopics objectAtIndex:indexPath.row];
        ...
        if (activities){
            Activity *activity;
            if ([_report.type isEqualToString:kDaily]){
                activity = _report.dailyActivities[indexPath.row];
            } else {
                activity = _report.activities[indexPath.row];
            }
            if (self.delegate && [self.delegate respondsToSelector:@selector(showActivityWithId:)]){
                [self.delegate showActivityWithId:activity.identifier];
            }
        } else {
            //comment cell
        }
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    if ([textField isKindOfClass:[BHPersonnelCountTextField class]]) {
        if ([(BHPersonnelCountTextField*)textField personnelType] == kUserHours && textField.tag < _report.reportUsers.count) {
            ReportUser *reportUser = [_report.reportUsers objectAtIndex:textField.tag];
        ...
        } else if ([(BHPersonnelCountTextField*)textField personnelType] == kSubcontractorCount && textField.tag < _report.reportSubs.count) {
            ReportSub *reportSub = [_report.reportSubs objectAtIndex:textField.tag];
...
- (void)removeUser:(UIButton*)button {
    if (_report.reportUsers.count > button.tag){
        ReportUser *reportUser = [_report.reportUsers objectAtIndex:button.tag];
        if (reportUser && ![_report.identifier isEqualToNumber:[NSNumber numberWithInt:0]]) {
            NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
            [parameters setObject:_report.identifier forKey:@"report_id"];
...
- (void)removeSubcontractor:(UIButton*)button {
    ...
    if (_report.reportSubs.count > button.tag){
        ReportSub *reportSub = [_report.reportSubs objectAtIndex:button.tag];
        if (![_report.identifier isEqualToNumber:[NSNumber numberWithInt:0]]) {
...
- (void)removeTopic:(UIButton*)button {
    SafetyTopic *topic;
    for (SafetyTopic *t in _report.safetyTopics) {
    ...
    NSIndexPath *indexPathForDeletion = [NSIndexPath indexPathForRow:[_report.safetyTopics indexOfObject:topic] inSection:6];
    [_report removeSafetyTopic:topic];
    [topic MR_deleteInContext:[NSManagedObjectContext MR_defaultContext]];
    [[NSManagedObjectContext MR_defaultContext] MR_saveToPersistentStoreAndWait];
    //update the UI
    [_reportTableView deleteRowsAtIndexPaths:@[indexPathForDeletion] withRowAnimation:UITableViewRowAnimationFade];
    [_reportTableView endUpdates];
}
